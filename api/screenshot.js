...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    await page.goto(url, { waitUntil: "networkidle2", timeout: 30000 });
    await new Promise(r => setTimeout(r, 2000));
    const mobileShot = await page.screenshot({ type: "jpeg", quality: 85, fullPage: false });
...
function buildHTML(desktopB64, mobileB64, affLink, originalUrl) {
  // Escape affLink for safe embedding
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Mobile: swap to mobile screenshot */
  @media (max-width: 768px) {
    .page-bg {
      background-image: url('data:image/jpeg;base64,${mobileB64}');
    }
  }
  ...
  .btn-allow {
    background: #2e7d62;
    color: #fff;
    box-shadow: 0 4px 16px rgba(46,125,98,0.3);
  }
